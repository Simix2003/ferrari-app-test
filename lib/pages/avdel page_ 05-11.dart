// ignore_for_file: unused_local_variable, library_private_types_in_public_api, depend_on_referenced_packages, non_constant_identifier_names
...
class _AvdelPageState extends State<AvdelPage>
  ...
  void initState() {
    ...
    Future.delayed(Duration.zero, () {
      // Set the active page as 'Avdel Page'
      final activePageProvider =
          Provider.of<ActivePageProvider>(context, listen: false);
      activePageProvider.setActivePage('Avdel Page');

      if (activePageProvider.isActivePage('Avdel Page')) {
        // Start the timer only if the page is "Avdel Page"
        periodicTimer = Timer.periodic(const Duration(seconds: 10), (timer) {
          fetchDataFromBackend_Avdel();
        });
      }
    });
    // Use Future.delayed to set the active page after the build is complete
  }
      ...
      children: [
        ElevatedButton(
          ...
          style: ElevatedButton.styleFrom(
            shape: CircleBorder(),
            primary: colorValue ? Colors.green : Colors.grey[200],
          ),
          child: Container(
            width: 100,
            height: 100,
